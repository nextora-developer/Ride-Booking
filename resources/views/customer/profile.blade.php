...
@section('content')
    ...
    <div class="pb-24 space-y-6">
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div
                class="bg-white rounded-[2.2rem] p-5 sm:p-6
           shadow-[0_14px_34px_rgba(15,23,42,0.08)]
           border border-slate-200 min-w-0">

                <div class="flex flex-col items-center text-center">

                    {{-- Avatar --}}
                    <div
                        class="h-14 w-14 sm:h-16 sm:w-16 rounded-[1.8rem] bg-slate-900 text-white
                   flex items-center justify-center font-black text-xl sm:text-2xl
                   shadow-[0_18px_45px_rgba(15,23,42,0.18)]">
                        {{ $initial }}
                    </div>

                    {{-- Name --}}
                    <div class="mt-3 text-base sm:text-xl font-black text-slate-900 truncate">
                        {{ $u->name }}
                    </div>

                    {{-- Email --}}
                    <div class="text-xs sm:text-sm font-bold text-slate-600 truncate">
                        {{ $u->email }}
                    </div>
            ...
            <div
                class="bg-slate-900 rounded-[2.2rem] p-5 sm:p-6 text-white min-w-0
           shadow-[0_18px_45px_rgba(15,23,42,0.25)]">
                ...
                <div class="mt-2 sm:mt-3 text-2xl sm:text-4xl font-black tracking-tight text-red-500 truncate">
                    RM {{ number_format($u->credit_balance ?? 0, 2) }}
                </div>

                <div class="mt-1.5 sm:mt-2 text-[10px] sm:text-xs text-slate-400 font-bold">
                    未结清行程费用
                </div>

                <div class="mt-4 sm:mt-6">
                    <a href="{{ route('customer.credit.logs') }}"
                        class="inline-flex items-center gap-2 px-3 sm:px-4 py-2 rounded-xl
                   bg-white/10 hover:bg-white/20 text-[10px] sm:text-xs
                ...
                <div
                    ...
                    <div class="mt-6">
                        ...
                        <div class="group relative">
                            ...
                            <input name="full_name" value="{{ old('full_name', $u->full_name) }}" placeholder="与身份证一致的姓名"
                                class="w-full pl-12 pr-4 py-4 rounded-2xl bg-white/85 border border-slate-300/80
                                   focus:border-indigo-300 focus:bg-white focus:ring-4 focus:ring-indigo-500/10
                                   transition-all outline-none text-sm font-bold text-slate-900 placeholder:text-slate-500
                                   shadow-[0_14px_34px_rgba(15,23,42,0.10)]" />
