...
<head>
    ...
    <title>@yield('title', config('app.name') . ' - 管理后台')</title>
...
<body class="h-screen overflow-hidden font-sans antialiased text-slate-900 bg-gray-50">
        ...
        <aside x-cloak
            ...
            <div class="h-16 px-5 border-b border-gray-100 flex items-center justify-between shrink-0">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <div class="text-sm font-extrabold leading-4">管理后台</div>
                        <div class="text-xs text-gray-500">总控面板</div>
                    </div>
                </div>

                <button type="button"
                    class="lg:hidden inline-flex items-center justify-center h-9 w-9 rounded-xl border border-gray-200 hover:bg-gray-50"
                    @click="sidebarOpen = false" aria-label="关闭菜单">
                    <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <nav class="flex-1 overflow-y-auto p-4 space-y-1">
                ...
                <a href="{{ $safeRoute('admin.managers.index') }}" @click="sidebarOpen=false"
                    class="{{ $navLink }}
                               {{ request()->routeIs('admin.managers.*') ? $navActive : $navIdle }}
                               {{ $has('admin.managers.index') ? '' : 'opacity-50 cursor-not-allowed' }}"
                    @if (!$has('admin.managers.index')) @click.prevent @endif>
                    <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <main class="px-4 sm:px-6 lg:px-8 pb-24 lg:pb-10">
                ...
                @if ($errors->any())
                    <div class="mb-6 p-4 rounded-2xl bg-red-50 text-red-800 text-sm">
                        <div class="font-semibold mb-2">请检查以下错误：</div>
