...
<head>
    ...
    <title>@yield('title', config('app.name') . ' - Customer')</title>
            ...
            <div class="mx-auto max-w-7xl px-4 pb-[max(env(safe-area-inset-bottom),12px)]">
                <div
                    ...
                    <div class="grid grid-cols-4 gap-1 p-2">
                        ...
                        <a href="{{ route('customer.book') }}"
                            class="flex flex-col items-center justify-center gap-1 rounded-2xl px-2 py-2.5 transition-all
                            {{ $isBook ? 'bg-slate-900 text-white' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-50' }}">
                            <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="2"
                                stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round"
                            ...
                            <span class="text-[11px] font-extrabold">Book</span>
                        </a>

                        {{-- Orders --}}
                        <a href="{{ route('customer.orders.index') }}"
                            class="flex flex-col items-center justify-center gap-1 rounded-2xl px-2 py-2.5 transition-all
                            {{ $isOrder ? 'bg-slate-900 text-white' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-50' }}">
                            <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="2"
                                stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round"
                        ...
                        <div x-data="{ open: false }" class="relative">
                            <button @click="open = !open" @click.outside="open = false" type="button"
                                ...
                                {{ $isProfile ? 'bg-slate-900 text-white' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-50' }}">
                                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="2"
                                    stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                                ...
                                <div class="p-2">
                                    <div class="px-3 py-2">
                                        <div class="text-xs font-black text-slate-900 truncate">
                                            {{ auth()->user()->name }}</div>
                                        <div class="text-[11px] text-slate-400 font-semibold">Customer</div>
                                    </div>
                ...
                <div class="flex flex-col md:flex-row items-center justify-between gap-6">
                    <div class="flex items-center gap-2">
                        ...
                        <p class="text-[13px] text-slate-500 font-medium">
                            © {{ date('Y') }} {{ config('app.name') }}. Built for the road.
                        </p>
                    </div>
