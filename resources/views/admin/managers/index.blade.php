...
@section('header')
    <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
            <h1 class="text-3xl font-extrabold tracking-tight text-slate-900">经理</h1>
            <p class="mt-1 text-sm text-slate-500 font-semibold">管理经理账号与查看账号状态。</p>
        </div>
    ...
    <div class="rounded-3xl bg-white border border-gray-100 shadow-sm overflow-hidden">
        <div class="px-5 sm:px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            <div class="text-sm font-extrabold text-slate-900">
                经理列表
                <span class="ml-2 text-xs font-black text-slate-500">共 {{ $managers->total() }} 位</span>
            </div>
        </div>
            ...
            <table class="min-w-full">
                <thead class="bg-gray-50">
                    <tr class="text-left text-xs font-black uppercase tracking-widest text-slate-400">
                        <th class="px-5 sm:px-6 py-3">经理</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($managers as $m)
                        <tr class="hover:bg-gray-50/60 {{ !$m->is_active ? 'opacity-60' : '' }}">
                            ...
                            <td class="px-5 sm:px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div
                                        ...
                                        {{ strtoupper(mb_substr($m->name ?? 'M', 0, 1)) }}
                                    </div>
                                    <div class="min-w-0">
                            ...
                            <td class="px-5 sm:px-6 py-4">
                                <span
                                    class="inline-flex items-center px-3 py-1 rounded-full text-xs font-extrabold
        {{ $m->shift === 'day' ? 'bg-amber-50 text-amber-700' : 'bg-indigo-100 text-indigo-700' }}">
                                    {{ $m->shift === 'day' ? '白班' : '夜班' }}
                                </span>
                            </td>
                            ...
                            <td class="px-5 sm:px-6 py-4">
                                <div class="flex flex-col items-start gap-1">
                                    <form action="{{ route('admin.managers.toggle', $m) }}" method="POST">
                                        @csrf
                                        @method('PATCH')
                                        ...
                                        <button type="submit"
                                            ...
                                            {{ $m->is_active ? 'bg-emerald-500' : 'bg-rose-500' }}
                                            hover:ring-4 {{ $m->is_active ? 'hover:ring-emerald-500/10' : 'hover:ring-rose-500/10' }}">
                                    ...
                                    <span
                                        class="text-[9px] font-black uppercase tracking-widest {{ $m->is_active ? 'text-emerald-600' : 'text-rose-600' }}">
                                        {{ $m->is_active ? '正常' : '已暂停' }}
                                    </span>
                                </div>
                            </td>
                            ...
                            <td class="px-5 sm:px-6 py-4">
                                ...
                                <div class="text-xs text-slate-500 font-semibold">
                                    {{ optional($m->created_at)->format('h:i A') }}
                                </div>
                            </td>
                            ...
                            <td class="px-5 sm:px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    {{-- 查看 --}}
                                    <a href="{{ route('admin.managers.show', $m) }}"
                                    ...
                                    {{-- 编辑 --}}
                                    <a href="{{ route('admin.managers.edit', $m) }}"
